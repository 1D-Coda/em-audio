...
| 2 | Mathematical correctness | **5** | The claim order and its meet are defined explicitly before any proposition, so every property is one line from the definition. Verified by 885,828 exhaustive checks over all 9,840 source words of length ≤ 8, by a closed-form oracle that recomputes expected states without the evidence algebra, and by a 10,860-case differential against a second implementation using a different algorithm, with zero disagreements and zero support difference. |
...
| 4 | Reproducibility | **4** | One-command reproduction from a clean clone; pinned tool versions; checksum-pinned corpus fetch; frozen fixtures and frozen manifests; commit and tag in the manuscript; `SHA256SUMS`; auto-generated preflight report; `run_all.sh` exits non-zero on any conformance failure. Independent clean reproduction on a second machine has since been performed and is reported in Section 7.11, with the reproducer's raw outputs released in `results/independent/`. **Not a 5 only because** the Zenodo version and concept DOIs are not yet assigned and the repository is not yet public. |
...
and subtracts the decoded outputs. Across 280 probes over 7 operator
configurations, 52,005 output samples were influenced and none fell outside the
declared support on this machine; an independent run on a different FFmpeg build
puts 110 of them outside it, which Section 7.11 reports. The margins also justify the particular bands rather than
...
**One administrative condition remains before the submit button is pressed.**

1. ~~**Independent clean reproduction.**~~ Closed. D. A. Balderrama-Alvarez
   (Universidad de Sonora, ORCID 0009-0002-5180-0406) ran the packaged release
   twice on his own hardware. Eight of the ten compared result files reproduced
   with no deterministic difference and every headline count returned unchanged;
   the two that differed are reported as findings in Section 7.11 rather than
   absorbed. His unmodified outputs ship in `results/independent/`.
2. **Zenodo deposit.** Archive the tagged release and add the version and concept
   DOIs to the manuscript at proof stage. This still requires the repository to
   be published, which only the author can do.

Until that is done, the correct status is *submission-ready pending archive*,
not *submitted*.
