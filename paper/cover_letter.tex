...
\signature{A. Urias}
\address{Aurtech\\C.P. 83220, Hermosillo, Sonora, Mexico\\alex@aurtech.mx}
\begin{document}
...
re-encoded, can emerge described as captured-only --- with a valid signature,
an intact chain and a bit-identical waveform --- because a derived asset
inherits from its parent and a minimal implementation summarises that parent
from the boundaries of the retained region. For an examiner reading such an
...
Three things are worth flagging to the editors directly, because we would rather
