...
# Dependency preflight. An independent reproduction ran for twenty minutes before
...
# Once left outside the gate, so a regression in the defects it covers could
# leave the main command green.
...
# committed copy is present and the table builds, so the defect did not show;
...
# Optional second reader. Section 9 states single-validator transport as a
# limitation; this covers part of it, and is skipped rather than failed
...
# check must not end a third party's run in RUN FAILED. Its result file is what
# is read; a non-zero exit is printed and recorded rather than propagated.
